(function() {
    ...
    GesturePersistent.prototype.lastAttemptTime = function() {
        return window.localStorage.getItem('lastTimeAttemptedUnlock')
    }
})();
...
(function() {
    ...
    GestureLock.prototype.dismiss = function() {
        document.getElementById('wrapper').className = "animated rotateOut";
        setTimeout("document.getElementById('wrapper').remove()", 2000);
    }
...
(function() {
    window.GestureLockManager = function() { 
        this.showGestureLockAfter = 5 * 60;
    };

    GestureLockManager.prototype.isMobileDevice = function() {
        return ('ontouchstart' in document.documentElement);
    }

    GestureLockManager.prototype.showGestureLockIfNeeded = function() {
        if (this.isMobileDevice() == false) {
            return
        }

        var result = this.shouldShowGestureLock();
        if (this.shouldShowGestureLock() == true) {
           new GestureLock({
                chooseType: 3
            });
        }
    ...
    GestureLockManager.prototype.shouldShowGestureLock = function() {
        var lastTimeAttemptedUnlock = (new GesturePersistent()).lastAttemptTime();
        var currentTime = Date.parse(new Date());
        return (currentTime - lastTimeAttemptedUnlock) > this.showGestureLockAfter * 1000
    }
})();

function showGestureLockIfNeeded() {
    var gestureManager = new GestureLockManager;
    gestureManager.showGestureLockIfNeeded();
}